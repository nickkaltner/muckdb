...
muckdb session tile <name> --name TILE --db <db> (--view V | --sql "SQL")
        ...
        [--label COL]  (map: per-point label shown in the hover tooltip; on a connections map it labels each arc AND names its endpoint markers, so hovering a point lists the routes through it; timeline: the text drawn in each bar; sequence: the message text)
