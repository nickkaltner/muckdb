...
test.describe('array cell formatting', () => {
  test('empty array shows "—", never a bare unit suffix', async ({ page }) => {
    ...
    // Non-empty cells apply the suffix once, after the values. A four-item
    // fixture guards against the former universal three-item inline cap.
    const filled = page.locator('table.preview .cell-json', { hasText: '10, 100, 1,000, 10,000' }).first();
    await expect(filled).toContainText('10, 100, 1,000, 10,000 Gbps');
  ...
  test('list-column stats value filters by equality, not containment (no collapse)', async ({ page }) => {
    ...
    // The `sizes` stat card shows whole-array values ("[10, 100]" on 149 rows).
    // Clicking one must filter by equality (CAST = '[10, 100]' → 149 rows), not
    ...
    await expect(page.locator('.stat-card', { hasText: /rows 149\b/ }).first()).toBeVisible();
  });
});
